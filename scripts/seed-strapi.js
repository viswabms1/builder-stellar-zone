...
async function createPageContentType(strapiUrl, apiToken) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      error(`Failed to create content type: ${errorData.error?.message || response.statusText}`);
      return false;
    }
  } catch (err) {
...
async function createRoles(strapiUrl, apiToken) {
  ...
  const roles = [
    ...
    { name: 'Engineering', description: 'Can manage Engineering department pages' },
    { name: 'Health Sciences', description: 'Can manage Health Sciences department pages' },
    { name: 'Commerce & Management', description: 'Can manage Commerce & Management pages' },
    { name: 'Applied Sciences', description: 'Can manage Applied Sciences pages' },
    { name: 'Humanities & Social Sciences', description: 'Can manage HSS pages' },
    { name: 'Law', description: 'Can manage Law department pages' },
    { name: 'Journalism', description: 'Can manage Journalism department pages' },
    { name: 'Mathematics', description: 'Can manage Mathematics department pages' },
...
async function createVisionMissionContent(strapiUrl, apiToken) {
  ...
  try {
    const response = await fetch(`${strapiUrl}/api/pages`, {
      ...
      body: JSON.stringify({
        data: {
          ...
          slug: 'vision-mission',
          description: "Guiding DSU's transition from a legacy institution to a future-ready, AI-first university",
    ...
    if (response.ok) {
      ...
      const errorData = await response.json();
      error(`Failed to create content: ${errorData.error?.message || response.statusText}`);
      return false;
    }
  } catch (err) {
...
async function main() {
  ...
  if (!strapiUrl) {
    strapiUrl = await askQuestion('Enter your Strapi URL (e.g., https://your-project.strapiapp.com): ');
  }

  if (!apiToken) {
    apiToken = await askQuestion('Enter your Strapi API Token (must have admin role): ');
  }
  ...
  info(`API Token: ${apiToken.substring(0, 10)}...${apiToken.substring(apiToken.length - 10)}`);
  console.log('\n');
