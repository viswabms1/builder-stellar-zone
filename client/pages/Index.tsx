            ...
            {schoolsData.map((school, index) => {
              ...
              const cardElement = (
                ...
                >
                  ...
                  {/* Content Container */}
                  <div
                    className={`${style.bg} ${style.border} border border-t-0 rounded-none p-2 flex flex-col justify-center items-center h-24 w-full backdrop-blur-sm`}
                  >
                    <div className={`text-center ${school.key === 'medical' ? 'space-y-0.25' : 'space-y-1.5'}`}>
                      <h3 className={`${school.key === 'medical' ? 'text-[9px] md:text-[10px] leading-tight' : 'text-base md:text-lg'} font-bold text-foreground font-display hover:text-brand-magenta transition-colors text-center ${school.key === 'medical' ? 'line-clamp-4' : 'line-clamp-2'}`}>
                        {school.title}
                      </h3>
                      {school.campus && (
                        <p className={`${school.key === 'medical' ? 'text-[8px] py-0 px-1' : 'text-xs py-0.5 px-2'} font-semibold text-foreground/70 bg-foreground/10 rounded-full inline-block`}>
                          {school.campus}
                        </p>
                      )}
                    </div>
