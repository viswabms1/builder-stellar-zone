...
import { computerApplicationsFaculty } from "@/data/computer-applications-faculty";
import {
  ChevronRight,
} from "lucide-react";
...
export default function ComputerApplicationsFaculty() {
  const leadership = useMemo(
    () => {
      ...
      return computerApplicationsFaculty.sort((a, b) => rank(a.title) - rank(b.title));
    },
    [],
  );
...
type FacultyEntry = (typeof computerApplicationsFaculty)[number];
...
function FacultyGridCard({ faculty }: { faculty: FacultyEntry }) {
  return (
    <Link
      to={`/academics/computer-applications/faculty/${faculty.slug}`}
      ...
      <div className="text-center space-y-4 mb-16">
        <div>
          <h2 className="text-4xl md:text-5xl font-bold font-display text-foreground mb-3">
            Computer Applications Faculty Directory
          </h2>
        ...
        <p className="max-w-2xl mx-auto text-lg md:text-xl text-foreground/75 leading-relaxed">
          Meet our brilliant faculty members—researchers, innovators, and mentors steering education and industry partnerships in computer applications.
        </p>
      </div>
