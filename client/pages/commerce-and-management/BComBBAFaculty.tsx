    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="relative px-3 py-8">
        <div className="mx-auto max-w-7xl">
          <LeadershipSection leadership={leadership} programName={programName} />
        </div>
      </div>
    </div>
  );
...
function FacultyGridCard({ faculty, programPath }: { faculty: FacultyEntry; programPath: string }) {
  return (
    <Link
      to={`/academics/commerce-and-management${programPath}/faculty/${faculty.slug}`}
...
function LeadershipSection({ leadership, programName }: LeadershipSectionProps) {
  if (leadership.length === 0) {
    return null;
  }

  const programPath = programName === "B.Com" ? "/bcom" : "/bba";
    ...
    <section className="py-10 md:py-28">
      <div className="text-center space-y-4 mb-16">
        ...
        <p className="max-w-2xl mx-auto text-lg md:text-xl text-foreground/75 leading-relaxed">
          Meet our distinguished faculty—commerce and business educators, researchers, and mentors dedicated to providing comprehensive knowledge and professional excellence.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 auto-rows-max">
        {leadership.map((faculty) => (
          <FacultyGridCard key={faculty.slug} faculty={faculty} programPath={programPath} />
        ))}
      </div>
    </section>
  );
