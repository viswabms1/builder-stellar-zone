...
function DepartmentNoticeBoard({ department }: { department: string }) {
  ...
  useEffect(() => { if (eventItems.length === 0) return; const interval = setInterval(() => setCurrentEventIndex((prev) => (prev + 1) % eventItems.length), 6000); return () => clearInterval(interval); }, [eventItems.length]);
  useEffect(() => { if (newsItems.length === 0) return; const interval = setInterval(() => setCurrentNewsIndex((prev) => (prev + 1) % newsItems.length), 6000); return () => clearInterval(interval); }, [newsItems.length]);
  useEffect(() => { if (announcements.length === 0) return; const interval = setInterval(() => setCurrentAnnouncementIndex((prev) => (prev + 1) % announcements.length), 6000); return () => clearInterval(interval); }, [announcements.length]);
  const renderCarousel = (title: string, items: CarouselItem[], currentIndex: number, setCurrentIndex: (idx: number) => void, category: "Event" | "News" | "Announcement") => { const styles = getCategoryStyles(category); if (items.length === 0) return <div className="space-y-4"><div className={`flex items-center gap-2 px-4 py-3 rounded-xl border-l-4 ${styles.borderColor} ${styles.bgColor}`}><h3 className={`headline-4 font-display ${styles.textColor}`}>{title}</h3></div><p className="text-xs text-foreground/60 italic p-4 text-center">No items to display</p></div>; const currentItem = items[currentIndex]; return <div className="space-y-4"><div className={`flex items-center gap-2 px-4 py-3 rounded-xl border-l-4 ${styles.borderColor} ${styles.bgColor}`}><h3 className={`headline-4 font-display ${styles.textColor}`}>{title}</h3><Badge className="ml-auto text-xs">{currentIndex + 1} / {items.length}</Badge></div><Card className="group overflow-hidden rounded-2xl border-2 border-border/30 bg-card/40 backdrop-blur-sm">{currentItem.image && <div className="relative h-48"><img src={currentItem.image} alt={currentItem.title} className="w-full h-full object-cover" /></div>}<CardContent className="p-4 space-y-3"><h4 className="font-display font-semibold text-sm text-foreground mb-2">{currentItem.title}</h4><p className="text-xs text-foreground/70">{currentItem.description}</p><div className="flex items-center justify-between pt-2 border-t border-border/20"><span className="text-xs font-semibold text-foreground/60">{currentItem.date}</span></div></CardContent></Card><div className="flex items-center justify-between gap-2"><div className="flex gap-1">{items.map((_, idx) => <button key={idx} onClick={() => setCurrentIndex(idx)} className={`h-1.5 rounded-full ${idx === currentIndex ? `${styles.dotColor} w-6` : "bg-border/40 w-1.5"}`} />)}</div><div className="flex gap-1"><Button variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => setCurrentIndex((prev) => (prev - 1 + items.length) % items.length)}>←</Button><Button variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => setCurrentIndex((prev) => (prev + 1) % items.length)}>→</Button></div></div></div>; };
  return <section id="notice-board" className="px-3 py-8"><div className="mx-auto max-w-7xl"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3"><div>{renderCarousel("Events", eventItems, currentEventIndex, setCurrentEventIndex, "Event")}</div><div>{renderCarousel("News", newsItems, currentNewsIndex, setCurrentNewsIndex, "News")}</div><div>{renderCarousel("Announcements", announcements, currentAnnouncementIndex, setCurrentAnnouncementIndex, "Announcement")}</div></div></div></section>;
}
