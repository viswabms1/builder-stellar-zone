  ...
  const leadership = useMemo(
    () => {
      const rank = (title: string) => {
        ...
        if (/Professor/i.test(title) && !/Associate/i.test(title) && !/Assistant/i.test(title) && !/of Practice/i.test(title)) return 3;
        if (/Professor of Practice/i.test(title)) return 4;
        if (/Associate Professor/i.test(title)) return 5;
    ...
    >
      <div className="relative overflow-hidden bg-slate-200 dark:bg-slate-800">
        ...
        <div className="absolute bottom-0 w-full p-2 text-white">
          <p className="text-xs font-bold line-clamp-2 group-hover:text-brand-blue transition-colors">{faculty.name}</p>
      ...
      <div className="text-center space-y-4 mb-16">
        <div>
          <h2 className="text-4xl md:text-5xl font-bold font-display text-foreground mb-3">
            Aerospace Engineering Faculty Directory
          </h2>
          <div className="w-20 h-1.5 bg-gradient-to-r from-brand-blue to-brand-cyan rounded-full mx-auto" />
        </div>
        <p className="max-w-2xl mx-auto text-lg md:text-xl text-foreground/75 leading-relaxed">
          Meet our distinguished faculty members—pioneers, researchers, and educators advancing aerospace education and innovation.
        </p>
      </div>
