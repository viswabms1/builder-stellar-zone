...
import { journalismFaculty } from "@/data/journalism-faculty";
import {
  ChevronRight,
} from "lucide-react";

export default function JournalismFaculty() {
  const leadership = useMemo(
    () => {
      ...
      return journalismFaculty.sort((a, b) => rank(a.title) - rank(b.title));
    },
    [],
  );
...
type FacultyEntry = (typeof journalismFaculty)[number];
...
function FacultyGridCard({ faculty }: { faculty: FacultyEntry }) {
  return (
    <Link
      to={`/academics/journalism-mass-communication/faculty/${faculty.slug}`}
    ...
    <section className="py-10 md:py-28">
      <div className="text-center space-y-4 mb-16">
        ...
        <p className="max-w-2xl mx-auto text-lg md:text-xl text-foreground/75 leading-relaxed">
          Meet our distinguished faculty—journalists, scholars, and mentors dedicated to excellence in media education and communication.
        </p>
      </div>
